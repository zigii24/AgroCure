...
$query_gejala = "SELECT * FROM gejala ORDER BY kode";
...
// Ambil data aturan dengan join
$query = "SELECT a.*, p.nama as nama_penyakit, p.kode as kode_penyakit, 
                 g.nama as nama_gejala, g.kode as kode_gejala 
          FROM aturan a 
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['tambah'])) {
    ...
    $query_cek = "SELECT * FROM aturan WHERE id_penyakit=$id_penyakit AND id_gejala=$id_gejala";
    $result_cek = mysqli_query($conn, $query_cek);
    
    if (mysqli_num_rows($result_cek) > 0) {
        $error = 'Aturan untuk penyakit dan gejala ini sudah ada!';
    } else {
        $query = "INSERT INTO aturan (id_penyakit, id_gejala, cf_pakar) VALUES ($id_penyakit, $id_gejala, $cf_pakar)";
        ...
        if (mysqli_query($conn, $query)) {
            ...
            $error = 'Gagal menambahkan aturan: ' . mysqli_error($conn);
        }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['edit'])) {
    ...
    $query = "UPDATE aturan SET id_penyakit=$id_penyakit, id_gejala=$id_gejala, cf_pakar=$cf_pakar WHERE id=$id";
    
    if (mysqli_query($conn, $query)) {
        $_SESSION['success'] = 'Aturan berhasil diupdate!';
        header('Location: aturan.php');
        exit;
    } else {
        $error = 'Gagal mengupdate aturan: ' . mysqli_error($conn);
    }
}
...
<head>
    ...
    <title>Kelola Aturan - AGROCURE</title>
...
<body>
    ...
    <header class="admin-header">
        <nav class="admin-nav">
            ...
            <ul class="nav-links">
                ...
                <li><a href="aturan.php" class="active"><i class="fas fa-link"></i>Aturan</a></li>
    ...
    <section class="main-content">
        ...
        <div class="welcome-section">
            <h2>Kelola Aturan CF Pakar 🔗</h2>
            <p>Manajemen hubungan antara penyakit dan gejala dengan nilai Certainty Factor</p>
        </div>
        ...
        <div class="card">
            <h3><i class="fas fa-plus-circle"></i><?php echo isset($_GET['edit']) ? ' Edit Aturan' : ' Tambah Aturan Baru'; ?></h3>
            <form method="POST" action="aturan.php">
                <?php if (isset($_GET['edit'])): 
                    $edit_id = $_GET['edit'];
                    $query_edit = "SELECT * FROM aturan WHERE id=$edit_id";
                    $result_edit = mysqli_query($conn, $query_edit);
                    $aturan_edit = mysqli_fetch_assoc($result_edit);
                ?>
                    <input type="hidden" name="id" value="<?php echo $aturan_edit['id']; ?>">
                    ...
                    <select id="id_penyakit" name="id_penyakit" class="form-control" required>
                        ...
                        <?php foreach ($penyakit_list as $penyakit): ?>
                            <option value="<?php echo $penyakit['id']; ?>" 
                                <?php echo (isset($aturan_edit) && $aturan_edit['id_penyakit'] == $penyakit['id']) ? 'selected' : ''; ?>>
                ...
                <div class="form-group">
                    ...
                    <select id="id_gejala" name="id_gejala" class="form-control" required>
                        <option value="">Pilih Gejala</option>
                        <?php foreach ($gejala_list as $gejala): ?>
                            <option value="<?php echo $gejala['id']; ?>" 
                                <?php echo (isset($aturan_edit) && $aturan_edit['id_gejala'] == $gejala['id']) ? 'selected' : ''; ?>>
                                <?php echo $gejala['kode'] . ' - ' . $gejala['nama']; ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                </div>
                ...
                <div class="form-group">
                    <label for="cf_pakar">CF Pakar (0.0 - 1.0)</label>
                    <input type="number" id="cf_pakar" name="cf_pakar" class="form-control" 
                           step="0.1" min="0" max="1" 
                           value="<?php echo isset($aturan_edit) ? $aturan_edit['cf_pakar'] : '0.8'; ?>" 
                ...
                <div style="display: flex; gap: 1rem; flex-wrap: wrap;">
                    <button type="submit" class="btn">
                        <i class="fas fa-save"></i><?php echo isset($_GET['edit']) ? ' Update Aturan' : ' Tambah Aturan'; ?>
                    </button>
                    
                    <?php if (isset($_GET['edit'])): ?>
                        <a href="aturan.php" class="btn btn-secondary">
        ...
        <!-- Daftar Aturan -->
        <div class="card">
            <h3><i class="fas fa-list"></i> Daftar Aturan CF Pakar</h3>
            
            <?php if (empty($aturan_list)): ?>
                <div class="empty-state">
                    <i class="fas fa-inbox"></i>
                    <h3>Belum ada data aturan</h3>
                    <p>Mulai dengan menambahkan aturan baru menggunakan form di atas</p>
                </div>
            <?php else: ?>
                    ...
                    <table>
                        ...
                        <tbody>
                            <?php foreach ($aturan_list as $aturan): ?>
                                <tr>
                                    <td>
                                        <strong><?php echo $aturan['kode_penyakit']; ?></strong> - 
                                        <?php echo $aturan['nama_penyakit']; ?>
                                    </td>
                                    <td>
                                        <strong><?php echo $aturan['kode_gejala']; ?></strong> - 
                                        <?php echo $aturan['nama_gejala']; ?>
                                    </td>
                                    <td>
                                        <span class="cf-badge"><?php echo $aturan['cf_pakar']; ?></span>
                                    </td>
                                    <td>
                                        <div style="display: flex; gap: 0.5rem;">
                                            <a href="aturan.php?edit=<?php echo $aturan['id']; ?>" class="btn-edit">
                                                <i class="fas fa-edit"></i> Edit
                                            </a>
                                            <a href="aturan.php?delete=<?php echo $aturan['id']; ?>" class="btn-delete" 
